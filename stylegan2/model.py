...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        if self.activation:
            ...
            # print('weight'+str(self.weight.dtype))
            # print('input'+str(input.dtype))
            # print(self.scale.dtype)# float
            # print('bias'+str(self.bias.dtype))
            # print(self.lr_mul.dtype)# int
            out = F.linear(
                input.float(), self.weight * self.scale, bias=self.bias * self.lr_mul
            )
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        # print('styles',styles.size()) # [1, 1, 512] -> [1, 3, 512]
        # len(latent) == 1
        if len(styles) < 2:
            # print(1)
            ...
            if styles[0].ndim < 3:
                ...
                # styles[0].ndim = 2
                # 换的
                # latent1 = styles[0][0].unsqueeze(0).unsqueeze(0).repeat(1, 4, 1)
                # latent2 = styles[0][1].unsqueeze(0).unsqueeze(0).repeat(1, 4, 1)
                # latent3 = styles[0][2].unsqueeze(0).unsqueeze(0).repeat(1, 10, 1)
                # latent = torch.cat((latent1,latent2,latent3), dim = 1)
        ...
        out = self.input(latent) # self.input 是constant input
